...
geneType <- read.table("gencode.v38.annotation.geneannotation.txt", header = T)
...
# of the genes with more than one class, remove "lncRNA" and "unprocessed_pseudogene"
...
RedCorrected <- Redundant[!Redundant$Class %in% c("lncRNA", "unprocessed_pseudogene"), ]

# Manually checked that these class identifications are wrong
RedCorrected <- RedCorrected %>% filter(RedCorrected$GeneSymbolClass != "5S_rRNA_rRNA_pseudogene")
RedCorrected <- RedCorrected %>% filter(RedCorrected$GeneSymbolClass != "FAM153B_transcribed_processed_pseudogene")
RedCorrected <- RedCorrected %>% filter(RedCorrected$GeneSymbolClass != "GABARAPL3_TEC")
...
write.csv(correctedGeneType, "gencode.v40.annotation.geneannotation_corrected.csv", row.names = F)
